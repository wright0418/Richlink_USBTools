            ...
            if ser.in_waiting:
                ...
                if msg:
                    ...
                    if type == 'MRG-MSG':
                        ...
                        set_publish_window.write_event_value(
                            ('-READ_THREAD-', 'MSG', data[0]), None)
                    elif type == "MSAA-MSG":
                        set_group_window.write_event_value(
                            ('-READ_THREAD-', 'MSG', data[0]), None)
        sleep(0.1)
...
def set_subscribe_group():
    while True:
        event, values = set_group_window.read()

        if event in (sg.WINDOW_CLOSED, 'EXIT'):
            break
        elif event == "set_group":
            # 執行設定群動作
            write_mesh_AtCmd(
                f'MSAA {values["uid"]} 0 0x04005D {values["group"]}')
        elif event[0] == '-READ_THREAD-':
            name = event[1]
            value = event[2]
            if name == "MSG":
                if value == "SUCCESS":
                    sg.popup_timed('Set Group SUCESS',
                                   button_type=5, auto_close_duration=2)
                else:
                    sg.popup_timed('Set Group Fail',
                                   button_type=5, auto_close_duration=2)
    set_group_window.close()


def set_publish():
    while True:
        event, values = set_publish_window.read()
        if event in (sg.WINDOW_CLOSED, 'EXIT'):
            break
        elif event == "set_publish":
            # 執行設定推撥動作
            write_mesh_AtCmd(
                f'MPAS {values["uid"]} 0 0x04005D {values["ToUid"]} 0')
            ...
            if name == "MSG":
                if value == "SUCCESS":
                    sg.popup_timed('Set Publish SUCESS',
                                   button_type=5, auto_close_duration=2)
                else:
                    sg.popup_timed('Set Publish Fail',
                                   button_type=5, auto_close_duration=2)

    set_publish_window.close()
...
layout = [
    ...
    [sg.Button('Connect', key='CONNECT', disabled=False),
     sg.Button('Close'), sg.Button('Unbind All'),
     sg.Text('', key='ROLE', text_color='red'),
     sg.Button("Set Subscribe Group", key="set_group_window"),
     sg.Button("Set Publish To")],
...
# check event from bind  thread
    ...
    elif event == "set_group_window":
        window.disable()
        # 新增加一個子窗口的 layout
        set_group_layout = [
            [sg.Text("裝置 UID"), sg.Combo(list(device_list.keys()), key="uid"), sg.Text(
                "訂閱 群組"), sg.Combo(["0xC000", "0xC001", "0xC002", "0xC003"], key="group")],
            [sg.Button("Set", key="set_group"), sg.B("EXIT")]
        ]
        # 新增加一個子窗口的窗口對象
        set_group_window = sg.Window("Set Subscribe Group", set_group_layout)
        # 打開子窗口
        set_subscribe_group()
        window.enable()
    elif event == "Set Publish To":
        window.disable()
        # 新增加一個子窗口的 layout
        set_publish_layout = [
            [sg.Text("裝置 UID"), sg.Combo(list(device_list.keys()), key="uid"), sg.Text(
                "推撥給"), sg.Combo(list(device_list.keys())+["0xC000", "0xC001", "0xC002", "0xC003"], key="ToUid")],
            [sg.Button("Set", key="set_publish"), sg.B("EXIT")]
        ]
        # 新增加一個子窗口的窗口對象
        set_publish_window = sg.Window("Set Publish", set_publish_layout)
        # 打開子窗口
        set_publish()
        window.enable()
    ...
    if ble_connected:
        if event == 'Unbind All':
            confirm = sg.popup('Are you sure to un-binding all device?',
                               title='Confirmation', button_type=sg.POPUP_BUTTONS_YES_NO)
            if confirm == 'Yes':
                write_mesh_AtCmd('NR')
                device_list = {}
                # 將字典存成檔案
